...
class RSASAConnector(phantom.BaseConnector):
    ...
    def _login(self):
        ...
        try:
            ...
            return self.set_status(phantom.APP_ERROR, f"Unable to connect to server. Error: {e!s}")
        ...
        if not data:
            if self.get_action_identifier() == self.ACTION_ID_TEST_ASSET_CONNECTIVITY:
                self.save_progress(consts.RSASA_ERR_TEST_CONNECTIVITY)
            return self.set_status(phantom.APP_ERROR, "Could not find INCIDENT_MANAGEMENT type devices configured, can't continue")
        ...
        try:
            ...
            return self.set_status(
                phantom.APP_ERROR,
                f"Could not find INCIDENT_MANAGEMENT type device named '{config[consts.RSASA_JSON_INCIDENT_MANAGER]}'. Can't continue",
            )
    ...
    def _make_rest_call(self, endpoint, result, params=None, headers=None):
        ...
        if not (200 <= r.status_code <= 399):
            ...
            return RetVal(
                result.set_status(phantom.APP_ERROR, f"Call failed with HTTP Code: {r.status_code}. Reason: {r.reason}. Details: {detail}"), None
            )
        ...
        if success is not True:
            ...
            return RetVal(result.set_status(phantom.APP_ERROR, f"Call failed, details: {message}"), resp_json)
    ...
    def _restart_service(self, param):
        ...
        return action_result.set_status(
            phantom.APP_ERROR, "This action has been deprecated. Please use the NetWitness Logs and Packets 'restart device' action instead."
        )
    ...
    def _get_incidents(self, action_result, limit, start_time, end_time, sort="ASC"):
        ...
        query_params = {"limit": limit, "start": 0, "sort": f'[{{"property": "created", "direction": "{sort}"}}]'}

        if start_time and end_time:
            query_params["filter"] = f'[{{"property": "created", "value": [{start_time}, {end_time}]}}]'

        endpoint = f"/ajax/incidents/{self._inc_mgnt_id}"
    ...
    def _get_alerts(self, action_result, incident, limit):
        ...
        endpoint = f"/ajax/alerts/{self._inc_mgnt_id}"
        query_params = {
            "start": 0,
        ...
        if incident:
            query_params["filter"] = f'[{{"property": "incidentId", "value": "{incident}"}}]'
        else:
            query_params["filter"] = (
                f'[{{"property": "alert.timestamp", "value": [{consts.RSASA_DEFAULT_START_TIME}, {int(time.time()) * 1000}]}}]'
            )
    ...
    def _on_poll(self, param):
        ...
        for incident in incidents:
            ...
            if phantom.is_fail(ret_val):
                self.debug_print(f"get alert failed with: {action_result.get_message()}")
            ...
            for alert in alerts:
                ...
                if phantom.is_fail(ret_val):
                    self.debug_print(f"get event failed with: {action_result.get_message()}")
